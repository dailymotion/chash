...
static void test_end(char *format, ...)
{
    ...
    if (test_status)
    {
        printf("fail (%.3fms - %d", time_spent, test_status);
    }
    else
    {
...
int main(int argc, char **argv)
{
    ...
    int           index, count, size1, size2, target, lookups[TARGETS];
    u_char        *serialized1, *serialized2;
    char          buffer[32], **lookup;
    ...
    test_start("freeze");
    test_step(chash_freeze(&context) < 0 ? -1 : 0, NULL);
    test_end("continuum count is %d", chash_freeze(&context));

    test_start("unfreeze");
    test_step(chash_unfreeze(&context) < 0 ? -1 : 0, NULL);
    test_end(NULL);

    test_start("serialize");
    test_step((size1 = chash_serialize(&context, &serialized1)) < 0 ? -1 : 0, NULL);
    test_end("serialized size is %d bytes", size1);

    chash_initialize(&context, 1);
    test_start("unserialize");
    test_step((count = chash_unserialize(&context, serialized1, size1)) < 0 ? -1 : 0, NULL);
    test_end("continuum count is %d", count);

    test_start("serialize coherency");
    test_step((size2 = chash_serialize(&context, &serialized2)) < 0 ? -1 : 0, NULL);
    test_step(size1 != size2 || memcmp(serialized1, serialized2, size1) ? -1 : 0, NULL);
    test_end(NULL);

    test_start("file_serialize");
    test_step((size1 = chash_file_serialize(&context, "/tmp/chash.freeze")) < 0 ? -1 : 0, NULL);
    test_end("serialized size is %d bytes", size1);

    chash_initialize(&context, 1);
    test_start("file_unserialize");
    test_step((count = chash_file_unserialize(&context, "/tmp/chash.freeze")) < 0 ? -1 : 0, NULL);
    test_end("continuum count is %d", count);
    ...
    for (index = 0; index < CANDIDATES; index ++)
    {
        ...
        test_step((count = chash_lookup(&context, buffer, 1, &lookup)) < 0 ? -1 : 0, NULL);
    ...
    test_start("lookup deviation");
    count = 0;
    for (index = 0; index < TARGETS; index ++)
    {
        count += lookups[index];
    }
    mean = (double)count / (TARGETS - 20);
    for (index = 0; index < TARGETS; index ++)
    {
        if (lookups[index])
        {
    ...
    test_end("deviation is %.2f", sqrt(deviation / (TARGETS - 20)));
