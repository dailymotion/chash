...
#define chash_return(instance, status) (_chash_return(instance, status))

static int _chash_return(chash_object *instance, int status)
{
...
{
    chash_object *instance = (chash_object *)Z_OBJ_P(getThis());
    u_char       use;

    if (zend_parse_parameters(ZEND_NUM_ARGS(), "b", &use) == SUCCESS)
    {
...
PHP_METHOD(CHash, addTarget)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS(), "s|l", &target, &length, &weight) != SUCCESS || length == 0)
    {
...
PHP_METHOD(CHash, removeTarget)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS(), "s", &target, &length) != SUCCESS || length == 0)
    {
        RETURN_LONG(chash_return(instance, CHASH_ERROR_INVALID_PARAMETER));
    }
...
PHP_METHOD(CHash, setTargets)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS(), "a", &targets) != SUCCESS)
    {
        RETURN_LONG(chash_return(instance, CHASH_ERROR_INVALID_PARAMETER));
    }
    ...
    for (zend_hash_internal_pointer_reset_ex(Z_ARRVAL_P(targets), &position);
         zend_hash_get_current_key_ex(Z_ARRVAL_P(targets), &target, &length, &position) == HASH_KEY_IS_STRING &&
         zend_hash_get_current_data_ex(Z_ARRVAL_P(targets), &position) == SUCCESS &&
...
PHP_METHOD(CHash, serialize)
{
    ...
    ZVAL_STRINGL((char *)serialized, size, 1);
    free(serialized);
}
...
PHP_METHOD(CHash, unserialize)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS(), "s", &serialized, &length) != SUCCESS || length == 0)
    {
        RETURN_LONG(chash_return(instance, CHASH_ERROR_INVALID_PARAMETER));
    }
...
PHP_METHOD(CHash, serializeToFile)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS(), "s", &path, &length) != SUCCESS || length == 0)
    {
        RETURN_LONG(chash_return(instance, CHASH_ERROR_INVALID_PARAMETER));
    }
...
PHP_METHOD(CHash, lookupList)
{
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS(), "s|l", &candidate, &length, &count) != SUCCESS || length == 0 || count < 1)
    {
...
static void chash_free(chash_object *instance)
{
    zend_object_std_dtor(&(instance->zo));
...
PHP_MINIT_FUNCTION(chash)
{
    ...
    chash_class_entry = zend_register_internal_class(&class_entry);
    ...
    chash_memory_exception = zend_register_internal_class_ex(&class_entry, zend_exception_get_default());
    INIT_CLASS_ENTRY(class_entry, "CHashIOException", NULL);
    chash_io_exception = zend_register_internal_class_ex(&class_entry, zend_exception_get_default());
    INIT_CLASS_ENTRY(class_entry, "CHashInvalidParameterException", NULL);
    chash_invalid_parameter_exception = zend_register_internal_class_ex(&class_entry, zend_exception_get_default());
    INIT_CLASS_ENTRY(class_entry, "CHashNotFoundException", NULL);
    chash_not_found_exception = zend_register_internal_class_ex(&class_entry, zend_exception_get_default());
